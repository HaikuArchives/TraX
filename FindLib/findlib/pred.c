...
/* Put fnmatch.h after defs.h, since defs.h includes unistd.h, which on
   ...
   and do it's own way.  Also, use our local copy, not one from a system
   include directory. */
#include "fnmatch.h"
#include "modetype.h"
#include <sys/wait.h>
...
#else
#ifdef __BEOS__
# define ST_NBLOCKS( statp ) ( off_t ) ( ( ( statp ) ->st_size + 512 - 1 ) / 512 )
#else
...
boolean
pred_crtime ( pathname, stat_buf, pred_ptr )
char *pathname ;
struct stat *stat_buf ;
struct predicate *pred_ptr ;
{
	...
	{
		case COMP_GT:
			if ( stat_buf->st_crtime > ( time_t ) pred_ptr->args.info.l_val )
				return ( true ) ;
			break ;
		case COMP_LT:
			if ( stat_buf->st_crtime < ( time_t ) pred_ptr->args.info.l_val )
		...
		case COMP_EQ:
			if ( ( stat_buf->st_crtime >= ( time_t ) pred_ptr->args.info.l_val )
        				&& ( stat_buf->st_crtime < ( time_t ) pred_ptr->args.info.l_val
             				+ DAYSECS ) )
				return ( true ) ;
			break ;
	}
